...
Program:   3D Slicer
Module:    $RCSfile: vtkMRMLPointCloudNode.cxx,v $
...
//----------------------------------------------------------------------------
vtkMRMLNodeNewMacro(vtkMRMLPointCloudNode);

//----------------------------------------------------------------------------
vtkMRMLPointCloudNode::vtkMRMLPointCloudNode()
{
...
//----------------------------------------------------------------------------
void vtkMRMLPointCloudNode::Copy(vtkMRMLNode *anode)
{
  int disabledModify = this->StartModify();
  this->Superclass::Copy(anode);
  vtkMRMLPointCloudNode* modelNode = vtkMRMLPointCloudNode::SafeDownCast(anode);
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::ProcessMRMLEvents ( vtkObject *caller,
                                           unsigned long event,
                                           void *callData )
{
  ...
  if (this->PolyDataConnection &&
    ...
    this->InvokeEvent(vtkMRMLPointCloudNode::PolyDataModifiedEvent, NULL);
    }
}

//----------------------------------------------------------------------------
vtkMRMLPointCloudDisplayNode* vtkMRMLPointCloudNode::GetPointCloudDisplayNode()
{
  return vtkMRMLPointCloudDisplayNode::SafeDownCast(this->GetDisplayNode());
}

//----------------------------------------------------------------------------
void vtkMRMLPointCloudNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkMRMLPointCloudNode::SetAndObservePolyData(vtkPolyData *polyData)
{
  if (polyData == 0)
    {
...
//---------------------------------------------------------------------------
vtkPolyData* vtkMRMLPointCloudNode::GetPolyData()
{
...
::SetPolyDataConnection(vtkAlgorithmOutput *newPolyDataConnection)
{
  ...
  this->StorableModifiedTime.Modified();
  this->Modified();
  this->InvokeEvent( vtkMRMLPointCloudNode::PolyDataModifiedEvent , this);
}

//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::AddPointScalars(vtkDataArray *array)
{
  this->AddScalars(array, vtkAssignAttribute::POINT_DATA);
}

//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::AddCellScalars(vtkDataArray *array)
{
  this->AddScalars(array, vtkAssignAttribute::CELL_DATA);
}

//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::AddScalars(vtkDataArray *array, int location)
{
  if (array == NULL)
    {
    return;
  ...
  vtkDebugMacro("PointCloud node has " << numScalars << " scalars now, "
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::RemoveScalars(const char *scalarName)
{
  if (scalarName == NULL)
    {
...
//---------------------------------------------------------------------------
const char * vtkMRMLPointCloudNode::GetActivePointScalarName(int type)
{
  if (this->GetPolyData() == NULL ||
...
//---------------------------------------------------------------------------
const char * vtkMRMLPointCloudNode::GetActiveCellScalarName(int type)
{
  if (this->GetPolyData() == NULL ||
...
//---------------------------------------------------------------------------
bool vtkMRMLPointCloudNode::HasPointScalarName(const char* scalarName)
{
  if (this->GetPolyData() == NULL ||
      this->GetPolyData()->GetPointData() == NULL)
    {
...
//---------------------------------------------------------------------------
bool vtkMRMLPointCloudNode::HasCellScalarName(const char* scalarName)
{
  if (this->GetPolyData() == NULL ||
      this->GetPolyData()->GetCellData() == NULL)
    {
...
//---------------------------------------------------------------------------
int vtkMRMLPointCloudNode::GetAttributeTypeFromString(const char* typeName)
{
...
//---------------------------------------------------------------------------
int vtkMRMLPointCloudNode::SetActivePointScalars(const char *scalarName, int attributeType)
{
  if (this->GetPolyData() == NULL)
    {
    return -1;
...
//---------------------------------------------------------------------------
int vtkMRMLPointCloudNode::SetActiveCellScalars(const char *scalarName, int attributeType)
{
  if (this->GetPolyData() == NULL)
    {
    return -1;
...
//---------------------------------------------------------------------------
int vtkMRMLPointCloudNode::CompositeScalars(const char* backgroundName, const char* overlayName,
                                       ...
                                       int reverseOverlay)
{
    ...
    if (colorNode->GetID() != NULL)
      {
      this->GetPointCloudDisplayNode()->SetAndObserveColorNodeID(colorNode->GetID());
      this->GetPointCloudDisplayNode()->SetScalarRange(-overlayMax, overlayMax);
      }
    ...
    // make them active
    this->GetPointCloudDisplayNode()->SetActiveScalarName(composedName.c_str());
...
//---------------------------------------------------------------------------
bool vtkMRMLPointCloudNode::CanApplyNonLinearTransforms()const
{
  return true;
}

//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::ApplyTransform(vtkAbstractTransform* transform)
{
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::GetRASBounds(double bounds[6])
{
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::TransformBoundsToRAS(double inputBounds_Local[6], double outputBounds_RAS[6])
{
...
//---------------------------------------------------------------------------
vtkMRMLStorageNode* vtkMRMLPointCloudNode::CreateDefaultStorageNode()
{
  return vtkMRMLStorageNode::SafeDownCast(vtkMRMLPointCloudStorageNode::New());
}

//----------------------------------------------------------------------------
void vtkMRMLPointCloudNode::CreateDefaultDisplayNodes()
{
  if (vtkMRMLPointCloudDisplayNode::SafeDownCast(this->GetDisplayNode())!=NULL)
    {
  ...
  if (this->GetScene()==NULL)
    {
    vtkErrorMacro("vtkMRMLPointCloudNode::CreateDefaultDisplayNodes failed: scene is invalid");
    return;
    }
  vtkNew<vtkMRMLPointCloudDisplayNode> dispNode;
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::OnNodeReferenceAdded(vtkMRMLNodeReference *reference)
{
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::OnNodeReferenceModified(vtkMRMLNodeReference *reference)
{
   this->UpdateDisplayNodePolyData(vtkMRMLDisplayNode::SafeDownCast(reference->GetReferencedNode()));
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode::UpdateDisplayNodePolyData(vtkMRMLDisplayNode *dnode)
{
  vtkMRMLPointCloudDisplayNode* modelDisplayNode =
    vtkMRMLPointCloudDisplayNode::SafeDownCast(dnode);
  if (modelDisplayNode)
    {
...
::SetPolyDataToDisplayNodes()
{
  ...
  for (int n=0; n<ndisp; n++)
    {
    vtkMRMLPointCloudDisplayNode *dnode = vtkMRMLPointCloudDisplayNode::SafeDownCast(
...
//---------------------------------------------------------------------------
void vtkMRMLPointCloudNode
::SetPolyDataToDisplayNode(vtkMRMLPointCloudDisplayNode* modelDisplayNode)
{
...
//---------------------------------------------------------------------------
bool vtkMRMLPointCloudNode::GetModifiedSinceRead()
{
